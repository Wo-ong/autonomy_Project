...
input_data_1 = pd.read_sql("SELECT * FROM User_20_1", con)
input_data_2 = pd.read_sql("SELECT * FROM User_20_2", con)
input_data_3 = pd.read_sql("SELECT * FROM User_20_3", con)


input_data_1_schema = input_data_1.iloc[-1]
if input_data_1_schema[-1] == 1:
    input_data_1['sex_plus'] = 0
else:
    input_data_1['sex_plus'] = 1


result_etc = pd.concat([input_data_1, input_data_2], axis=1)
result_etc = pd.concat([result_etc, input_data_3], axis=1)


gen_result_etc = result_etc[['height', 'weight', 'bodyfat', 'waist', 'flexibility', 'muscle', 'alacrity', 'bloodpressure', 'sex', 'sex_plus']]
input_list_etc = list(gen_result_etc.iloc[-1].values)


x = input_list_etc
x = pd.DataFrame(x)
x = x.transpose()
x.columns = ['ITEM_F001','ITEM_F002','ITEM_F003','ITEM_F004','ITEM_F012','ITEM_F019',
...
#마무리운동
last_result1 = df_aaar.columns[cc[0]]
last_result2 = df_aaar.columns[cc[1]]
last_result3 = df_aaar.columns[cc[2]]
